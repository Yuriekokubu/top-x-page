...
function render(contents: Movie[]) { // add
  ...
  for (let content of contents) {
    ...
    li.appendChild(
      document.createTextNode(content.name + " : " + content.star)
    );
...
// convert number of star to 
function toStar(star: number): string {
  return '⭐️'.repeat(star)
}

function renderByLang(lang = 'TH'): void {
  const renderData = data
    ...
    .map(v => (
      {
        ...v,
        name: (lang === 'EN') ? v.nameEN : v.nameTH,
        star: toStar(v.star)
      }
    ))
  render(renderData);
